// src/components/tripulantes/NewTripulanteModal.tsx
import React from 'react';
import { Formik } from 'formik';
import * as Yup from 'yup';
import type { Tripulante } from '../../types/tripulante';

type Props = {
  open: boolean;
  onClose: () => void;
  onCreated: (t: Tripulante) => void;
};
...
const schema = Yup.object({
  ...
  base: Yup.string().min(3).max(3).required('Requerido'),
  estado: Yup.mixed<'ACTIVO' | 'BAJA_TEMPORAL'>().oneOf(['ACTIVO', 'BAJA_TEMPORAL']).required(),
});

export function NewTripulanteModal({ open, onClose, onCreated }: Props) {
  if (!open) return null;
    ...
    <div style={backdrop}>
      <div style={modal}>
        ...
        <Formik
          ...
          onSubmit={(values) => {
            const newTripulante: Tripulante = {
              id: crypto.randomUUID(),
              ...values,
            };
            onCreated(newTripulante);
            onClose();
          }}
        >
          {(f) => (
            <form onSubmit={f.handleSubmit} style={{ display: 'grid', gap: 12 }}>
              <Field
                ...
                error={f.touched.nombre ? f.errors.nombre : undefined}
              />
              ...
              <Field
                ...
                onChange={f.handleChange}
                error={f.touched.codigo ? f.errors.codigo : undefined}
              />
              ...
              <div style={{ display: 'grid', gap: 6 }}>
                <label>Estado</label>
                <select name="estado" value={f.values.estado} onChange={f.handleChange} style={input}>
...
// estilos simples (puedes migrarlos a CSS)
